...
def parsejs(genetable):
    ...
    with open("knetminer_api.json", "r") as jf:
        with open(genetable, "w") as g:
            for line in jf:
                content = json.loads(line) #deserialise content of json, which will be dictionary object.
                #print(type(content))
...
if __name__ == "__main__":
    #0) Provide necessary parameter options
    '''FileType was not used as the function resformat() already has with open(file). Therefore the input parameter is a string.'''
    ...
    parser.add_argument("--f", help="gapit results file with csv extension")
    parser.add_argument("--p", help="a chosen logP threshold for filtering snps", type=int)
